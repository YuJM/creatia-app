...
module Web
  class RolesController < TenantBaseController
    ...
    def create
      ...
      if @role.save
        ...
        respond_to do |format|
          format.html { redirect_to web_organization_role_path(@organization, @role), notice: '역할이 생성되었습니다.' }
          format.turbo_stream
        end
      else
        @available_permissions = Permission.all.group_by(&:resource)
    ...
    def edit
      unless @role.editable?
        redirect_to web_organization_roles_path(@organization), alert: '시스템 역할은 수정할 수 없습니다.'
        return
      end
    ...
    def update
      ...
      if @role.update(role_params)
        ...
        respond_to do |format|
          format.html { redirect_to web_organization_role_path(@organization, @role), notice: '역할이 수정되었습니다.' }
          format.turbo_stream
        end
      else
        @available_permissions = Permission.all.group_by(&:resource)
    ...
    def destroy
      if @role.destroyable?
        ...
        respond_to do |format|
          format.html { redirect_to web_organization_roles_path(@organization), notice: '역할이 삭제되었습니다.' }
          format.turbo_stream { render turbo_stream: turbo_stream.remove(dom_id(@role)) }
        end
      else
        respond_to do |format|
          format.html { redirect_to web_organization_roles_path(@organization), alert: '사용 중인 역할은 삭제할 수 없습니다.' }
    ...
    def duplicate
      ...
      if @new_role.persisted?
        redirect_to web_edit_organization_role_path(@organization, @new_role), notice: '역할이 복제되었습니다.'
      else
        redirect_to web_organization_roles_path(@organization), alert: '역할 복제에 실패했습니다.'
      end
    end
