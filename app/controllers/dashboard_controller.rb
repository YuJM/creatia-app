...
class DashboardController < TenantBaseController
  ...
  def index
    ...
    # 현재 활성 스프린트
    @active_sprint = policy_scope(Sprint).active.first
  ...
  def calculate_dashboard_metrics
    tasks = policy_scope(Task).where(created_at: @start_date..@end_date)
    sprints = policy_scope(Sprint).where(start_date: @start_date..@end_date)
  ...
  def calculate_organization_burndown_data
    ...
    (30.days.ago.to_date..Date.current).each do |date|
      remaining_tasks = policy_scope(Task).where('created_at <= ? AND (status != ? OR updated_at > ?)', 
                                                date.end_of_day, 'done', date.end_of_day).count
      
      # 이상적인 번다운 계산 (선형 감소)
      total_days = 30
      elapsed_days = (date - 30.days.ago.to_date).to_i
      total_initial_tasks = policy_scope(Task).where(created_at: 30.days.ago.beginning_of_day..30.days.ago.end_of_day).count
  ...
  def calculate_velocity_trend(date_range = nil)
    ...
    range.beginning_of_week.step(range.end_of_week, 1.week) do |week_start|
      ...
      tasks_completed = policy_scope(Task)
                       .done
  ...
  def calculate_team_burndown_data(date_range = nil)
    ...
    data = []
    range.each do |date|
      remaining = policy_scope(Task).where('created_at <= ? AND (status != ? OR updated_at > ?)', 
                                          date.end_of_day, 'done', date.end_of_day).count
  ...
  def calculate_completion_rates(date_range = nil)
    ...
    %w[low medium high urgent].each do |priority|
      priority_tasks = policy_scope(Task).where(priority: priority, created_at: range)
    ...
    current_organization.users.active.each do |user|
      user_tasks = policy_scope(Task).where(assigned_user: user, created_at: range)
    ...
    # 일별 완료율
    range.each do |date|
      day_tasks = policy_scope(Task).where(created_at: date.beginning_of_day..date.end_of_day)
  ...
  def calculate_dashboard_alerts
    ...
    # 지연된 작업 경고
    overdue_count = policy_scope(Task).overdue.count
    if overdue_count > 0
      alerts << {
        id: "overdue_tasks",
        type: "overdue",
    ...
    # 높은 우선순위 작업 알림
    urgent_count = policy_scope(Task).urgent.where.not(status: 'done').count
    if urgent_count > 0
      alerts << {
        id: "urgent_tasks",
        type: "priority",
  ...
  def recent_activities
    ...
    # 최근 완료된 작업들
    recent_completed_tasks = policy_scope(Task).done
    ...
    # 새로 생성된 작업들
    recent_new_tasks = policy_scope(Task).where(created_at: 24.hours.ago..Time.current)
                                        .includes(:assigned_user)
