# app/models/mongodb/mongo_sprint_v2.rb
module Mongodb
  class MongoSprintV2
    include Mongoid::Document
    include Mongoid::Timestamps
    include PostgresReference
    
    # MongoDB 컬렉션 이름 설정
    store_in collection: "sprints_v2"
    
    # ===== Core References (PostgreSQL UUIDs) =====
    field :organization_id, type: String  # UUID from PostgreSQL
    field :service_id, type: String       # UUID from PostgreSQL
    field :team_id, type: String          # UUID from PostgreSQL
    field :milestone_id, type: String     # MongoDB Milestone ID
    
    # ===== Sprint Definition =====
    field :name, type: String
    field :goal, type: String
    field :sprint_number, type: Integer
    field :status, type: String, default: 'planning' # planning, active, completed, cancelled
    
    # ===== Timeline =====
    field :start_date, type: Date
    field :end_date, type: Date
    field :working_days, type: Integer
    
    # ===== Capacity & Velocity =====
    field :team_capacity, type: Float # 총 가용 시간
    field :planned_velocity, type: Float # 계획 속도
    field :actual_velocity, type: Float # 실제 완료 포인트
    field :carry_over_velocity, type: Float # 이월된 포인트
    
    # ===== Sprint Planning =====
    field :committed_points, type: Float
    field :stretch_points, type: Float
    field :completed_points, type: Float
    field :spillover_points, type: Float
    
    # ===== User Snapshots (Embedded) =====
    field :created_by_snapshot, type: Hash, default: {}
    # {
    #   user_id: "uuid",
    #   name: "John Doe",
    #   email: "john@example.com",
    #   avatar_url: "...",
    #   snapshot_at: DateTime
    # }
    
    field :sprint_owner_snapshot, type: Hash, default: {}
    field :scrum_master_snapshot, type: Hash, default: {}
    
    # Team members with roles
    field :team_member_snapshots, type: Array, default: []
    # [{
    #   user_id: "uuid",
    #   name: "Jane Doe",
    #   email: "jane@example.com",
    #   avatar_url: "...",
    #   role_in_sprint: "developer",
    #   capacity_hours: 40,
    #   snapshot_at: DateTime
    # }]
    
    # ===== Daily Tracking =====
    field :daily_standups, type: Array, default: []
    # [{
    #   date: Date,
    #   attendees: [user_snapshots],
    #   blockers: [],
    #   notes: String,
    #   recorded_by_snapshot: { user_snapshot }
    # }]
    
    field :burndown_data, type: Array, default: []
    
    # ===== Sprint Ceremonies =====
    field :planning_session, type: Hash, default: {}
    # {
    #   date: DateTime,
    #   duration_minutes: Integer,
    #   attendees: [user_snapshots],
    #   facilitator_snapshot: { user_snapshot },
    #   notes: String,
    #   decisions: []
    # }
    
    field :review_session, type: Hash, default: {}
    field :retrospective, type: Hash, default: {}
    
    # ===== Task Management =====
    field :task_ids, type: Array, default: []
    field :total_tasks, type: Integer, default: 0
    field :completed_tasks, type: Integer, default: 0
    field :active_tasks, type: Integer, default: 0
    
    # ===== Health & Risk =====
    field :health_score, type: Float, default: 100.0
    field :risk_level, type: String, default: 'low'
    field :blockers, type: Array, default: []
    # [{
    #   id: "blocker-1",
    #   description: String,
    #   raised_by_snapshot: { user_snapshot },
    #   assigned_to_snapshot: { user_snapshot },
    #   created_at: DateTime,
    #   resolved_at: DateTime,
    #   resolution: String
    # }]
    
    field :scope_changes, type: Array, default: []
    # [{
    #   change_id: String,
    #   description: String,
    #   requested_by_snapshot: { user_snapshot },
    #   approved_by_snapshot: { user_snapshot },
    #   impact: String,
    #   created_at: DateTime
    # }]
    
    # ===== Integrations =====
    field :external_links, type: Hash, default: {}
    
    # ===== Indexes =====
    index({ organization_id: 1, status: 1 })
    index({ service_id: 1, sprint_number: 1 })
    index({ team_id: 1, start_date: -1 })
    index({ status: 1, end_date: 1 })
    index({ "created_by_snapshot.user_id": 1 })
    index({ "sprint_owner_snapshot.user_id": 1 })
    index({ end_date: 1 }, { expire_after_seconds: 31536000 }) # 1년 후 자동 삭제
    
    # ===== Validations =====
    validates :name, presence: true
    validates :organization_id, presence: true
    validates :start_date, presence: true
    validates :end_date, presence: true
    validates :status, inclusion: { in: %w[planning active completed cancelled] }
    validate :end_date_after_start_date
    
    # ===== Scopes =====
    scope :active, -> { where(status: 'active') }
    scope :completed, -> { where(status: 'completed') }
    scope :current, -> { where(status: 'active', :start_date.lte => Date.current, :end_date.gte => Date.current) }
    scope :by_owner, ->(user_id) { where("sprint_owner_snapshot.user_id" => user_id) }
    scope :by_team_member, ->(user_id) { where("team_member_snapshots.user_id" => user_id) }
    
    # ===== User Snapshot Methods =====
    def set_created_by(user)
      self.created_by_snapshot = create_user_snapshot(user)
    end
    
    def set_sprint_owner(user)
      self.sprint_owner_snapshot = create_user_snapshot(user)
    end
    
    def set_scrum_master(user)
      self.scrum_master_snapshot = create_user_snapshot(user)
    end
    
    def add_team_member(user, role: 'developer', capacity_hours: 40)
      snapshot = create_user_snapshot(user).merge(
        role_in_sprint: role,
        capacity_hours: capacity_hours
      )
      
      # 중복 방지
      self.team_member_snapshots.reject! { |m| m['user_id'] == user.id.to_s }
      self.team_member_snapshots << snapshot
    end
    
    def remove_team_member(user_id)
      self.team_member_snapshots.reject! { |m| m['user_id'] == user_id.to_s }
    end
    
    def add_blocker(description, raised_by:, assigned_to: nil)
      blocker = {
        id: "blocker-#{SecureRandom.hex(8)}",
        description: description,
        raised_by_snapshot: create_user_snapshot(raised_by),
        assigned_to_snapshot: assigned_to ? create_user_snapshot(assigned_to) : nil,
        created_at: DateTime.current,
        resolved_at: nil,
        resolution: nil
      }
      
      self.blockers << blocker
      blocker
    end
    
    def resolve_blocker(blocker_id, resolution, resolved_by:)
      blocker = blockers.find { |b| b['id'] == blocker_id }
      return unless blocker
      
      blocker['resolved_at'] = DateTime.current
      blocker['resolution'] = resolution
      blocker['resolved_by_snapshot'] = create_user_snapshot(resolved_by)
    end
    
    def add_scope_change(description, requested_by:, impact:)
      change = {
        change_id: "change-#{SecureRandom.hex(8)}",
        description: description,
        requested_by_snapshot: create_user_snapshot(requested_by),
        approved_by_snapshot: nil,
        impact: impact,
        created_at: DateTime.current,
        approved_at: nil
      }
      
      self.scope_changes << change
      change
    end
    
    def approve_scope_change(change_id, approved_by:)
      change = scope_changes.find { |c| c['change_id'] == change_id }
      return unless change
      
      change['approved_by_snapshot'] = create_user_snapshot(approved_by)
      change['approved_at'] = DateTime.current
    end
    
    def record_standup(date, attendees, notes: nil, blockers: [], recorded_by:)
      standup = {
        date: date,
        attendees: attendees.map { |user| create_user_snapshot(user) },
        blockers: blockers,
        notes: notes,
        recorded_by_snapshot: create_user_snapshot(recorded_by),
        created_at: DateTime.current
      }
      
      # 같은 날짜의 기존 스탠드업 대체
      self.daily_standups.reject! { |s| s['date'] == date }
      self.daily_standups << standup
    end
    
    def set_planning_session(date:, duration_minutes:, attendees:, facilitator:, notes: nil, decisions: [])
      self.planning_session = {
        date: date,
        duration_minutes: duration_minutes,
        attendees: attendees.map { |user| create_user_snapshot(user) },
        facilitator_snapshot: create_user_snapshot(facilitator),
        notes: notes,
        decisions: decisions
      }
    end
    
    # ===== Instance Methods =====
    def tasks
      return @tasks if @tasks
      @tasks = task_ids.any? ? MongoTask.where(:_id.in => task_ids) : MongoTask.none
    end
    
    def calculate_health_score
      score = 100.0
      
      # Factor 1: Progress vs Time elapsed
      if start_date && end_date && Date.current.between?(start_date, end_date)
        time_elapsed_percent = ((Date.current - start_date).to_f / (end_date - start_date + 1) * 100).round
        progress_percent = completed_tasks.to_f / total_tasks * 100 rescue 0
        
        progress_delta = progress_percent - time_elapsed_percent
        score -= [progress_delta.abs, 30].min if progress_delta < 0
      end
      
      # Factor 2: Active blockers
      active_blockers = blockers.select { |b| b['resolved_at'].nil? }
      score -= active_blockers.size * 5
      
      # Factor 3: Unapproved scope changes
      unapproved_changes = scope_changes.select { |c| c['approved_at'].nil? }
      score -= unapproved_changes.size * 3
      
      self.health_score = [score, 0].max
      self.risk_level = case health_score
                        when 80..100 then 'low'
                        when 50..79 then 'medium'
                        else 'high'
                        end
      
      health_score
    end
    
    private
    
    def create_user_snapshot(user)
      return {} unless user
      
      {
        user_id: user.id.to_s,
        name: user.name,
        email: user.email,
        avatar_url: user.respond_to?(:avatar_url) ? user.avatar_url : nil,
        department: user.respond_to?(:department) ? user.department : nil,
        position: user.respond_to?(:position) ? user.position : nil,
        snapshot_at: DateTime.current
      }
    end
    
    def end_date_after_start_date
      return unless start_date && end_date
      errors.add(:end_date, 'must be after start date') if end_date <= start_date
    end
  end
end