...
RSpec.describe Mongodb::MongoMongodb::MongoTask, type: :model do
  ...
  describe "조직별 데이터 격리 (MongoDB)" do
    context "태스크는 조직별로 격리된다" do
      it "태스크는 특정 조직에 속해야 한다" do
        # Given: 조직 컨텍스트에서 태스크 생성
        task = build(:mongo_task, organization: organization, title: 'Test Mongodb::MongoTask')
      ...
      it "현재 테넌트 컨텍스트의 태스크만 조회된다" do
        ...
        current_org_task = create(:mongo_task, organization: organization, title: 'Current Org Mongodb::MongoTask')
        
        other_org = create(:organization, subdomain: 'other-org')
        
        # 다른 조직 컨텍스트에서 태스크 생성
        ActsAsTenant.with_tenant(other_org) do
          create(:mongo_task, organization: other_org, title: 'Other Org Mongodb::MongoTask')
        end
        
        # When: 태스크 목록 조회 (현재 테넌트 컨텍스트에서)
        ActsAsTenant.with_tenant(organization) do
          tasks = Mongodb::MongoTask.all
      ...
      it "다른 조직의 태스크에는 직접 접근할 수 없다" do
        ...
        # 다른 조직 컨텍스트에서 태스크 생성
        ActsAsTenant.with_tenant(other_org) do
          other_task = create(:mongo_task, organization: other_org, title: 'Other Mongodb::MongoTask')
        end
        
        # When: 현재 조직 컨텍스트에서 다른 조직 태스크 검색
        found_task = Mongodb::MongoTask.find_by(id: other_task.id)
    ...
    context "조직 없이는 태스크를 생성할 수 없다" do
      it "조직이 필수이다" do
        ...
        ActsAsTenant.current_tenant = nil
        task = Mongodb::MongoTask.new(title: 'No Org Mongodb::MongoTask', organization: nil)
  ...
  describe "태스크 기본 속성" do
    context "유효한 태스크 생성" do
      it "제목과 조직으로 태스크를 생성할 수 있다" do
        # Given: 최소 필수 정보
        task = build(:mongo_task, organization: organization, title: 'Simple Mongodb::MongoTask')
        
        # When: 태스크 저장
        # Then: 성공적으로 생성됨
        expect(task).to be_valid
        expect(task.title).to eq('Simple Mongodb::MongoTask')
        expect(task.organization).to eq(organization)
      end

      it "기본값이 적절히 설정된다" do
        # When: 기본값으로 태스크 생성
        task = create(:mongo_task, organization: organization, title: 'Default Mongodb::MongoTask')
    ...
    context "태스크 제목 검증" do
      it "제목은 필수이다" do
        # Given: 제목 없는 태스크
        task = build(:mongo_task, organization: organization, title: nil)
      ...
      it "제목은 1-200자 사이여야 한다" do
        # Given: 너무 짧거나 긴 제목
        empty_title = build(:mongo_task, organization: organization, title: '')
        too_long = build(:mongo_task, organization: organization, title: 'A' * 201)
        valid_title = build(:mongo_task, organization: organization, title: 'A')
    ...
    context "상태 및 우선순위 검증" do
      it "유효한 상태만 허용한다" do
        ...
        valid_statuses.each do |status|
          # When: 각 상태로 태스크 생성
          task = build(:mongo_task, organization: organization, status: status)
      ...
      it "유효한 우선순위만 허용한다" do
        ...
        valid_priorities.each do |priority|
          # When: 각 우선순위로 태스크 생성
          task = build(:mongo_task, organization: organization, priority: priority)
      ...
      it "유효하지 않은 상태는 거부한다" do
        # Given: 유효하지 않은 상태
        task = build(:mongo_task, organization: organization, status: 'invalid_status')
  ...
  describe "태스크 할당" do
    context "사용자 할당" do
      it "조직 멤버에게 태스크를 할당할 수 있다" do
        ...
        task = create(:mongo_task, organization: organization, title: 'Assigned Mongodb::MongoTask')
      ...
      it "할당되지 않은 태스크는 assigned? 메서드가 false를 반환한다" do
        # Given: 할당되지 않은 태스크
        task = create(:mongo_task, organization: organization, assigned_user: nil)
  ...
  describe "태스크 스코프와 필터링" do
      let!(:todo_task) { create(:mongo_task, organization: organization, status: 'todo', priority: 'high') }
  let!(:progress_task) { create(:mongo_task, organization: organization, status: 'in_progress', priority: 'medium') }
  let!(:done_task) { create(:mongo_task, organization: organization, status: 'done', priority: 'low') }
  let!(:urgent_task) { create(:mongo_task, organization: organization, status: 'todo', priority: 'urgent') }

    context "상태별 스코프" do
      it "TODO 태스크만 필터링할 수 있다" do
        # When: TODO 태스크 조회
        todo_tasks = Mongodb::MongoTask.todo
      ...
      it "진행 중인 태스크만 필터링할 수 있다" do
        # When: 진행 중 태스크 조회
        in_progress_tasks = Mongodb::MongoTask.in_progress
      ...
      it "완료된 태스크만 필터링할 수 있다" do
        # When: 완료 태스크 조회
        done_tasks = Mongodb::MongoTask.done
    ...
    context "우선순위별 스코프" do
      it "높은 우선순위 태스크만 필터링할 수 있다" do
        # When: 높은 우선순위 태스크 조회
        high_priority_tasks = Mongodb::MongoTask.by_priority('high')
      ...
      it "긴급 우선순위 태스크만 필터링할 수 있다" do
        # When: 긴급 우선순위 태스크 조회
        urgent_tasks = Mongodb::MongoTask.by_priority('urgent')
    ...
    context "사용자별 스코프" do
      ...
      it "특정 사용자에게 할당된 태스크만 필터링할 수 있다" do
        # When: 특정 사용자 태스크 조회
        user_tasks = Mongodb::MongoTask.assigned_to(user)
    ...
    context "정렬 스코프" do
      it "태스크를 위치순으로 정렬할 수 있다" do
        ...
        ActsAsTenant.with_tenant(test_org) do
          task1 = create(:mongo_task, organization: test_org, title: 'Order Mongodb::MongoTask 1', position: 2)
          task2 = create(:mongo_task, organization: test_org, title: 'Order Mongodb::MongoTask 2', position: 1)
          task3 = create(:mongo_task, organization: test_org, title: 'Order Mongodb::MongoTask 3', position: 3)
          
          # When: 위치순 정렬
          ordered_tasks = Mongodb::MongoTask.ordered
          
          # Then: 위치순으로 정렬됨 (이 조직의 태스크만)
          expect(ordered_tasks.count).to eq(3)
          expect(ordered_tasks.map(&:title)).to eq(['Order Mongodb::MongoTask 2', 'Order Mongodb::MongoTask 1', 'Order Mongodb::MongoTask 3'])
  ...
  describe "태스크 기한 관리" do
    context "기한 설정" do
      it "기한을 설정할 수 있다" do
        ...
        task = create(:mongo_task, organization: organization, due_date: due_date)
      ...
      it "기한이 지난 태스크를 확인할 수 있다" do
        # Given: 기한이 지난 태스크
        overdue_task = create(:mongo_task, organization: organization, due_date: 1.day.ago, status: 'todo')
        current_task = create(:mongo_task, organization: organization, due_date: 1.day.from_now, status: 'todo')
      ...
      it "완료된 태스크는 기한이 지나도 overdue가 아니다" do
        ...
        completed_task = create(:mongo_task, organization: organization, due_date: 1.day.ago, status: 'done')
      ...
      it "기한이 임박한 태스크를 확인할 수 있다" do
        # Given: 내일까지인 태스크
        due_soon_task = create(:mongo_task, organization: organization, due_date: 1.day.from_now)
        not_due_soon_task = create(:mongo_task, organization: organization, due_date: 5.days.from_now)
    ...
    context "기한별 스코프" do
      let!(:overdue_task) { create(:mongo_task, organization: organization, due_date: 1.day.ago, status: 'todo') }
      let!(:due_today_task) { create(:mongo_task, organization: organization, due_date: Date.current) }
      let!(:future_task) { create(:mongo_task, organization: organization, due_date: 5.days.from_now) }

      it "기한이 지난 태스크만 조회할 수 있다" do
        # When: 기한 초과 태스크 조회
        overdue_tasks = Mongodb::MongoTask.overdue
      ...
      it "오늘까지인 태스크를 조회할 수 있다" do
        # When: 오늘까지인 태스크 조회
        due_today_tasks = Mongodb::MongoTask.due_today
    ...
    context "상태 표시명" do
      it "각 상태의 한국어 표시명을 제공한다" do
        ...
        status_displays.each do |status, display_name|
          # When: 상태 설정 및 표시명 조회
          task = create(:mongo_task, organization: organization, status: status)
    ...
    context "우선순위 표시명 및 색상" do
      it "각 우선순위의 한국어 표시명을 제공한다" do
        ...
        priority_displays.each do |priority, display_name|
          ...
          task = create(:mongo_task, organization: organization, priority: priority)
  ...
  describe "태스크 데이터 무결성" do
    it "위치 값은 숫자여야 한다" do
      # Given: 유효한 위치 값
      task = build(:mongo_task, organization: organization, position: 5)
    ...
    it "설명은 선택사항이다" do
      # Given: 설명 없는 태스크
      task = build(:mongo_task, organization: organization, description: nil)
    ...
    it "설명이 있으면 저장된다" do
      ...
      task = create(:mongo_task, organization: organization, description: description)
